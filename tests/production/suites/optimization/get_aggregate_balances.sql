...
WITH group_members AS (
    ...
    FROM member m, multiset_user u WHERE group_id = 1 AND m.user_id = u.id
),

purchase_owed AS (
    SELECT purchaser_user_id user_id, SUM(total_cost) amount
    FROM purchase p, group_members gm
    WHERE purchaser_group_id = 1 AND purchaser_group_id = group_id AND purchaser_user_id = user_id
...
purchase_owing AS (
    ...
    WHERE ps.purchase_id IN (SELECT id FROM purchase WHERE purchaser_group_id = 1)
