...
WITH group_members AS (
    ...
    FROM member m, multiset_user u WHERE group_id = 4 AND m.user_id = u.id
),
...
group_purchase_splits AS (
    ...
    WHERE ps.purchase_id IN (SELECT id FROM purchase WHERE purchaser_group_id = 4)
...
group_settlements AS (
    ...
    WHERE sender_group_id = 4 AND receiver_group_id = 4
...
uncondensed AS (
SELECT 
    from_user_id, to_user_id, 4 group_id, 
