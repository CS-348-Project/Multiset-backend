...
def update_grocery_list_handler(request, grocery_list: GroceryListUpdate):
    try:
        ...
        return JsonResponse({"status": "error", "message": "Error in updating grocery list"}, status=400)
...
def get_grocery_list_items_handler(request, grocery_list_id: int):
    try:
        ...
        return JsonResponse({"status": "error", "message": "Error in fetching grocery list items"}, status=400)
...
def toggle_grocery_list_item_handler(request, item_id: int):
    try:
        ...
        return JsonResponse({"status": "error", "message": "Error in completing grocery list item"}, status=400)
...
def delete_grocery_list_item_handler(request, item_id: int):
    try:
        ...
        return JsonResponse({"status": "error", "message": "Error in deleting grocery list item"}, status=400)